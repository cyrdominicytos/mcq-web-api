...
public class StudentController {
    ...
    @PostMapping
    public @ResponseBody ResponseEntity<StudentListDto> create(@RequestBody StudentDto dto){
        StudentListDto object = this.studentService.createStudent(dto);
        return ResponseEntity.status(HttpStatus.CREATED).body(object);
    }

    @PutMapping("/update/{id}")
    public @ResponseBody ResponseEntity<StudentListDto> update(@PathVariable Long id, @RequestBody StudentDto dto){
        StudentListDto object = this.studentService.updateStudent(id, dto);
        return ResponseEntity.status(HttpStatus.OK).body(object);
    }
